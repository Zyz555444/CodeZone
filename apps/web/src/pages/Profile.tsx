...
// 按日期聚合真实活动数 (用于热力图)
function buildActivityCounts(activities: { createdAt: number }[]): Map<string, number> {
...
export default function Profile() {
  ...
  const activityCounts = useMemo(() => buildActivityCounts(activities), [activities]);
            ...
            <div className="flex">
              ...
              >
                {WEEKS.flat().map((date, i) => {
                  const count = activityCounts.get(dateKey(date)) ?? 0;
