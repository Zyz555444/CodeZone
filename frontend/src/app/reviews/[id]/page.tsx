              ...
              <div className="grid gap-6 lg:grid-cols-3">
                <div className="lg:col-span-2 space-y-6">
                  <Card>
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        <CardTitle className="font-serif text-2xl text-neutral-10">
                          {review.title}
                        </CardTitle>
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${status.bgClass} ${status.textClass}`}>
                          {status.label}
                        </span>
                      </div>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      {comments.length === 0 ? (
                        <p className="text-neutral-7 text-sm py-4 text-center">
                          暂无评论
                        </p>
                      ) : (
                            ...
                            <div key={comment.id} className="flex gap-3">
                              ...
                              <div className="flex-1 space-y-1">
                                <div className="flex items-center gap-2">
                                  <span className="font-medium text-sm text-neutral-10">
                                ...
                                {(comment.fileId || comment.lineNumber != null) && (
                                  <p className="text-xs text-neutral-5 font-mono">
                            ...
                            <div className="flex gap-2">
                              <div className="flex-1">
                                <Input
                                  ...
                                  placeholder="文件路径 (可选)"
                                  className="text-sm h-8 bg-neutral-2"
                                />
                              </div>
                              <div className="w-24">
                                <Input
                ...
                <div className="space-y-6">
                  <Card>
                    <CardHeader>
                      <CardTitle className="font-serif text-lg">审查信息</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <div>
                        <div className="flex items-center gap-2 mb-3">
                          <User className="h-4 w-4 text-neutral-6" />
                          <span className="text-sm text-neutral-7">作者</span>
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-neutral-10">{review.author.username}</span>
                        </div>
                      </div>
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <span className="text-sm text-neutral-7">项目</span>
                        </div>
                        <span className="text-sm text-neutral-10">{review.project.name}</span>
                      </div>

                      <div>
                        <div className="flex items-center gap-2 mb-3">
                          <Clock className="h-4 w-4 text-neutral-6" />
                          <span className="text-sm text-neutral-7">创建时间</span>
                        </div>
                      ...
                      {isAuthorOrOwner && review.status !== 'CLOSED' && review.status !== 'MERGED' && (
                        <div className="pt-4 border-t border-neutral-4">
                          <h4 className="text-sm font-medium text-neutral-7 mb-3">操作</h4>
                          <div className="flex gap-2">
                            <Button
